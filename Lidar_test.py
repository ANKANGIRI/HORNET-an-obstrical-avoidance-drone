...
# LiDAR device path
PORT_NAME = "COM3"  # Update with actual port
...
def collect_lidar_data():
    ...
    try:
        ...
        for scan in lidar.iter_scans(max_buf_meas=10000):  #lidar.iter_measurements()
            
            for (strength, angle, distance) in scan:
                if angle > last_angle+2:
                    i = last_angle +1
                    while i<angle:
                        temp_scan.append((strength,i, d)) #d is out of range as no obstacles can be seen
                        i+=0.5
...
def update_certainity_grid(lidar_data):
    ...
    for _, angle, distance in lidar_data:
        ...
        if 0 <= x < BOARD_SIZE and 0 <= y < BOARD_SIZE:
            ...
            board[x,y] += 2  
            if board[x,y]>10:
                board[x,y]=10 # !! CREATING THIS CAP IS SOMEHOW MAKING THE OUTPUT FLICKER (but at 500 flicker reduced)
